...
export class VolleyballRulesValidator {
  ...
  validateVolleyballPosition(
    ...
    const playerZoneEntry = Object.entries(rotationMap).find(
      ([, pid]) => pid === playerId
    );
    if (!playerZoneEntry) {
  ...
  private validateOverlapRules(
    ...
    if (constraints.horizontalConstraints.leftOf) {
      ...
      if (rightPosition && position.x >= rightPosition.x) {
        violations.push(
          `Player ${playerId} must be to the left of player ${rightPlayer}`
        );
        overlapViolations.push({
          violatedWith: rightPlayer,
          type: "horizontal",
        });
      }
    }
    ...
    if (constraints.horizontalConstraints.rightOf) {
      ...
      if (leftPosition && position.x <= leftPosition.x) {
        violations.push(
          `Player ${playerId} must be to the right of player ${leftPlayer}`
        );
        overlapViolations.push({
          violatedWith: leftPlayer,
          type: "horizontal",
        });
      }
    }
    ...
    if (constraints.verticalConstraints.inFrontOf) {
      ...
      if (backPosition && position.y >= backPosition.y) {
        violations.push(
          `Player ${playerId} must be in front of player ${backPlayer}`
        );
    ...
    if (constraints.verticalConstraints.behind) {
      ...
      if (frontPosition && position.y <= frontPosition.y) {
        violations.push(
          `Player ${playerId} must be behind player ${frontPlayer}`
        );
  ...
  private validateZoneOverlapRules(
    ...
    if (constraints.horizontalConstraints.leftOf) {
      ...
      if (rightPosition && position.x >= rightPosition.x) {
        violations.push(
          `Player in zone ${playerZone} must be to the left of player in zone ${rightZone}`
        );
        overlapViolations.push({
          violatedWith: rightPlayerId,
          type: "horizontal",
        });
      }
    }
    ...
    if (constraints.horizontalConstraints.rightOf) {
      ...
      if (leftPosition && position.x <= leftPosition.x) {
        violations.push(
          `Player in zone ${playerZone} must be to the right of player in zone ${leftZone}`
        );
        overlapViolations.push({
          violatedWith: leftPlayerId,
          type: "horizontal",
        });
      }
    }
    ...
    if (constraints.verticalConstraints.inFrontOf) {
      ...
      if (backPosition && position.y >= backPosition.y) {
        violations.push(
          `Player in zone ${playerZone} must be in front of player in zone ${backZone}`
        );
        overlapViolations.push({
          violatedWith: backPlayerId,
          type: "vertical",
        });
      }
    }
    ...
    if (constraints.verticalConstraints.behind) {
      ...
      if (frontPosition && position.y <= frontPosition.y) {
        violations.push(
          `Player in zone ${playerZone} must be behind player in zone ${frontZone}`
        );
        overlapViolations.push({
          violatedWith: frontPlayerId,
          type: "vertical",
        });
      }
    }
  ...
  private validateBackRowAttackRestrictions(
    ...
    if (position.y < this.courtZones.attackLine) {
      violations.push(
        `Back row player ${playerId} cannot be positioned in front of the attack line for attacks`
      );
    }
  }
  ...
  isValidVolleyballMove(
    ...
    const validation = this.validateVolleyballPosition(
      toPosition,
      playerId,
      formation,
      allPositions
